...
/**
 *      \class      InterfaceETransactionsWorkflow
 *      \brief      Class of triggers for etransactions module
 */
class InterfaceETransactionsWorkflow
{
    var $db;
     ...
     */
    function InterfaceETransactionsWorkflow($DB)
    {
        $this->db = $DB;

        $this->name = preg_replace('/^Interface/i','',get_class($this));
        $this->family = "etransactions";
        $this->description = "Triggers of this module allows to manage ETransactions workflow";
        $this->version = 'dolibarr';            // 'development', 'experimental', 'dolibarr' or version
        $this->picto = 'etransactions@etransactions';
    }
	...
	function run_trigger($action, $object, $user, $langs, $conf)
    {
        ...
        if ($triggered)
        {
        	...
        	dol_syslog("ETransactions: Trigger '".$this->name."' for action '".$action."' launched by ".__FILE__." ref=".$object->ref);
            ...
            if ($result < 0)
            {
                dol_syslog('ETransactions: Invoice/order with specified reference does not exist, email containing payment link has not been sent');
            	return $result;
            }
            else
            {
            ...
            if (price2num($amountTransaction, 'MT') == 0 || !$needPayment){
                dol_syslog('ETransactions: Payment already done, email containing payment link has not been sent');
                return 0;
            }
            ...
            if ($item->mode_reglement_id != $cbId){
                dol_syslog('ETransactions: Invoice/order payment mode is not CB, can not send payment link email');
                return 0;                
            }
            ...
            $etransactions = new ETransactions($this->db);
            $result = $etransactions->fetch('', $token);
            if ($result == 0)
            {
				$etransactions->key = $token;
				$etransactions->type = $invoice ? 'invoice' : 'order';
				$etransactions->fk_object = $item->id;
				$etransactions->create($user);
            }
            
            $paymentLink = dol_buildpath('/etransactions/payment.php', 2).'?key='.$token;
            ...
            if (!$result)
            {
                ...
                if ($result){
                    dol_syslog('ETransactions: Email containing payment link has been correctly sent');
                }else{
                    dol_syslog('ETransactions: Error sending email containing payment link');
                }
                return $result;
            }
            else
            {
                dol_syslog('ETransactions: Error in creating email containing payment link');
                return $result;
            }
